...
export function BackofficeSuperAdminInvites() {
  const { user } = useAuth()
  ...
  const loadInvites = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const res = await api.listSuperAdminInvites()
  ...
  const handleSendInvite = async (e: React.FormEvent) => {
    ...
    try {
      await api.createSuperAdminInvite(inviteEmail.trim(), inviteFullName.trim())
      ...
      // mantém comportamento simples, semelhante ao BackofficeInvite atual
      alert(msg || 'Falha ao enviar convite.')
    } finally {
  ...
  const handleDelete = async (id: string) => {
    ...
    setActionId(id)
    try {
      await api.deleteSuperAdminInvite(id)
      await loadInvites()
    } catch {
  ...
  const handleResend = async (id: string) => {
    setActionId(id)
    try {
      await api.resendSuperAdminInvite(id)
      await loadInvites()
    } catch (e: unknown) {
  ...
  return (
    <PageContainer>
      ...
      <Typography variant="h4" sx={{ mb: 2 }}>Convites para super admin</Typography>
      ...
      <Paper variant="outlined" sx={{ p: 2, maxWidth: 420, mb: 3 }}>
        ...
        <Box component="form" onSubmit={handleSendInvite}>
          ...
          <TextField label="Nome completo" fullWidth required placeholder="Nome do super admin" value={inviteFullName} onChange={(e) => setInviteFullName(e.target.value)} sx={{ mb: 1.5 }} />
