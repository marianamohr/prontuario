...
// NewClient returns a WhatsApp client.
func NewClient(cfg Config) *Client {
	return &Client{cfg: cfg, client: &http.Client{}}
}

// SendReminder sends a reminder text to the given phone (E.164).
// rescheduleURL: link para confirmar presença ou remarcar (se vazio, não inclui link).
func (c *Client) SendReminder(phone, patientName, dateStr, timeStr, rescheduleURL string) error {
	if c.cfg.AccountSid == "" || c.cfg.AuthToken == "" || c.cfg.From == "" {
		return nil
	}
	body := fmt.Sprintf("Lembrete: Amanhã (%s) às %s você tem consulta agendada para %s.", dateStr, timeStr, patientName)
	if rescheduleURL != "" {
		body += fmt.Sprintf(" Para confirmar presença ou remarcar: %s", rescheduleURL)
	} else {
		body += " Confirme sua presença se possível."
	}
