...
func TestSendAppointmentReminders_PoolNil(t *testing.T) {
	ctx := context.Background()
	date := time.Date(2025, 2, 12, 0, 0, 0, 0, time.UTC)
	sent, skipped := SendAppointmentReminders(ctx, nil, date, nil)
	if sent != 0 || skipped != 0 {
		t.Errorf("pool nil: got sent=%d skipped=%d, want 0,0", sent, skipped)
	}
}

func TestSendAppointmentRemindersWithLister_PoolAndListerNil(t *testing.T) {
	ctx := context.Background()
	date := time.Date(2025, 2, 12, 0, 0, 0, 0, time.UTC)
	sent, skipped := SendAppointmentRemindersWithLister(ctx, nil, date, nil, nil)
	if sent != 0 || skipped != 0 {
		t.Errorf("pool and lister nil: got sent=%d skipped=%d, want 0,0", sent, skipped)
	}
}
...
func TestSendAppointmentRemindersWithLister_SenderNil_CountsSkipped(t *testing.T) {
	...
	rows := []repo.AppointmentReminderRow{
		{AppointmentID: uuid.New(), PatientName: "Maria", GuardianPhone: "+5511999990000", StartTime: time.Now()},
		{AppointmentID: uuid.New(), PatientName: "João", GuardianPhone: "+5511888880000", StartTime: time.Now()},
	}
...
func TestSendAppointmentRemindersWithLister_AllSent(t *testing.T) {
	...
	date := time.Date(2025, 2, 12, 0, 0, 0, 0, time.UTC)
	rows := []repo.AppointmentReminderRow{
		{AppointmentID: uuid.New(), PatientID: uuid.New(), PatientName: "Maria", GuardianID: uuid.New(), GuardianPhone: "+5511999990000", StartTime: time.Date(0, 0, 0, 14, 30, 0, 0, time.UTC)},
		{AppointmentID: uuid.New(), PatientID: uuid.New(), PatientName: "João", GuardianID: uuid.New(), GuardianPhone: "+5511888880000", StartTime: time.Date(0, 0, 0, 9, 0, 0, 0, time.UTC)},
	}
	lister := &mockLister{rows: rows}
...
func TestSendAppointmentRemindersWithLister_PartialFail(t *testing.T) {
	...
	rows := []repo.AppointmentReminderRow{
		{AppointmentID: uuid.New(), PatientName: "Maria", GuardianPhone: "+5511999990000", StartTime: time.Now()},
		{AppointmentID: uuid.New(), PatientName: "João", GuardianPhone: "+5511888880000", StartTime: time.Now()},
		{AppointmentID: uuid.New(), PatientName: "Pedro", GuardianPhone: "+5511777770000", StartTime: time.Now()},
	}
	lister := &mockLister{rows: rows}
...
func (m *mockLister) ListAppointmentsForReminder(_ context.Context, _ *pgxpool.Pool, _ time.Time) ([]repo.AppointmentReminderRow, error) {
	if m.err != nil {
		return nil, m.err
	}
	return m.rows, nil
